...
describe("getOrCreateSessionId", () => {
  it("generates and persists a new id when none exists", async () => {
    ...
    expect(window.localStorage.getItem("analytics_session_id")).toBe(id);
  });

  it("reuses an existing id instead of generating a new one", async () => {
    window.localStorage.setItem("analytics_session_id", "existing-session-id");
    const { service } = await loadService({ configured: true });
...
describe("trackArticleView", () => {
  it("sends the documented payload shape", async () => {
    window.localStorage.setItem("analytics_session_id", "session-1");
